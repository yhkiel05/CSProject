// CharTrain.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main(int argc, char** argv)
{
 ...
 //   imgTrainingNumbers = cv::imread("training_chars2.png");          // read in training numbers image
    imgTrainingNumbers = cv::imread("image-A.png");          // read in training numbers image
    ...
    for (int i = 0; i < ptContours.size(); i++) {                           // for each contour
        if (cv::contourArea(ptContours[i]) > MIN_CONTOUR_AREA) {                // if contour is big enough to consider
            ...
            if (intChar == 27) {        // if esc key was pressed
                ...
                cv::Mat matImageFlattenedFloat = matImageFloat.reshape(1, 1);       // flatten to 1 row
    ...
    knn->train(matTrainingImagesAsFlattenedFloats, cv::ml::ROW_SAMPLE, matClassificationInts);
   
    FileStorage fs("knn.xml", FileStorage::WRITE);
    knn->write(fs);
    fs.release();
